	...
	for (int i = 0; i < THREADS_PER_HASH; i += _PARALLEL_HASH)
	{
		...
		for (uint32_t a = 0; a < ACCESSES; a += 4)
		{
			...
			for (uint32_t b = 0; b < 4; b++)
			{
				...
				dag_val[0] = __ldg( &(d_dag[offset[0]].uint4s[thread_id]) );
				...
				for (int p = 0; p < (_PARALLEL_HASH-2); p+=1)
				{
					...
					dag_val[p+1] = __ldg( &(d_dag[offset[p+1]].uint4s[thread_id]) );
				...
				dag_val[_PARALLEL_HASH-1] = __ldg( &(d_dag[offset[_PARALLEL_HASH-1]].uint4s[thread_id]) );
